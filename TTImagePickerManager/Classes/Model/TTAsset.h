...
typedef NS_ENUM(NSUInteger, TTAssetType) {
    TTAssetTypeUnknow,
    TTAssetTypeImage,
    TTAssetTypeVideo,
    TTAssetTypeAudio
...
typedef NS_ENUM(NSUInteger, TTAssetSubType) {
    TTAssetSubTypeUnknow,
    TTAssetSubTypeImage,
    TTAssetSubTypeLivePhoto NS_ENUM_AVAILABLE_IOS(9_1),
    TTAssetSubTypeGIF
};

/// Status when download asset from iCloud
typedef NS_ENUM(NSUInteger, TTAssetDownloadStatus) {
    TTAssetDownloadStatusSucceed,
    TTAssetDownloadStatusDownloading,
    TTAssetDownloadStatusCanceled,
    TTAssetDownloadStatusFailed
};

@interface TTAsset : NSObject
@property(nonatomic, assign, readonly) TTAssetType assetType;
@property(nonatomic, assign, readonly) TTAssetSubType assetSubType;

- (instancetype)initWithPHAsset:(PHAsset *)phAsset;

@property(nonatomic, strong, readonly) PHAsset *phAsset;
@property(nonatomic, assign, readonly) TTAssetDownloadStatus downloadStatus; // 从 iCloud 下载资源大图的状态
