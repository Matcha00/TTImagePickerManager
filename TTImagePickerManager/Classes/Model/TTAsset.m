    ...
    if (self = [super init]) {
        ...
        switch (phAsset.mediaType) {
            ...
            case PHAssetMediaTypeVideo:
                _assetType = TTAssetTypeVideo;
                break;
            case PHAssetMediaTypeAudio:
                _assetType = TTAssetTypeAudio;
                break;
            default:
                _assetType = TTAssetTypeUnknow;
                break;
        }
    }
    return self;
...
- (UIImage *)originImage {
    ...
    [[[TTPhotoManager manager] phCachingImageManager] requestImageDataAndOrientationForAsset:_phAsset options:phImageRequestOptions resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, CGImagePropertyOrientation orientation, NSDictionary * _Nullable info) {
...
- (UIImage *)thumbnailWithSize:(CGSize)size {
    ...
    [[[TTPhotoManager manager] phCachingImageManager] requestImageForAsset:_phAsset
                                                                          targetSize:size
...
- (NSInteger)requestOriginImageWithCompletion:(void (^)(UIImage *result, NSDictionary<NSString *, id> *info))completion withProgressHandler:(PHAssetImageProgressHandler)phProgressHandler {
    ...
    return [[[TTPhotoManager manager] phCachingImageManager] requestImageDataAndOrientationForAsset:_phAsset options:imageRequestOptions resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, CGImagePropertyOrientation orientation, NSDictionary * _Nullable info) {
        if (completion) {
...
- (NSInteger)requestThumbnailImageWithSize:(CGSize)size completion:(void (^)(UIImage *result, NSDictionary<NSString *, id> *info))completion {
    ...
    imageRequestOptions.networkAccessAllowed = YES;
    // 在 PHImageManager 中，targetSize 等 size 都是使用 px 作为单位，因此需要对targetSize 中对传入的 Size 进行处理，宽高各自乘以 ScreenScale，从而得到正确的图片
    return [[[TTPhotoManager manager] phCachingImageManager] requestImageForAsset:_phAsset targetSize:CGSizeMake(size.width, size.height) contentMode:PHImageContentModeAspectFill options:imageRequestOptions resultHandler:^(UIImage *result, NSDictionary *info) {
          if (completion) {
...
- (NSInteger)requestPreviewImageWithCompletion:(void (^)(UIImage *result, NSDictionary<NSString *, id> *info))completion withProgressHandler:(PHAssetImageProgressHandler)phProgressHandler {
    ...
    imageRequestOptions.networkAccessAllowed = YES; // 允许访问网络
    imageRequestOptions.progressHandler = phProgressHandler;
    return [[[TTPhotoManager manager] phCachingImageManager] requestImageForAsset:_phAsset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFill options:imageRequestOptions resultHandler:^(UIImage *result, NSDictionary *info) {
        if (completion) {
            completion(result, info);
        }
    }];
...
- (NSInteger)requestLivePhotoWithCompletion:(void (^)(PHLivePhoto *livePhoto, NSDictionary<NSString *, id> *info))completion withProgressHandler:(PHAssetImageProgressHandler)phProgressHandler {
    if ([[PHCachingImageManager class] instancesRespondToSelector:@selector(requestLivePhotoForAsset:targetSize:contentMode:options:resultHandler:)]) {
        ...
        return [[[TTPhotoManager manager] phCachingImageManager] requestLivePhotoForAsset:_phAsset targetSize:CGSizeMake(0, 0) contentMode:PHImageContentModeDefault options:livePhotoRequestOptions resultHandler:^(PHLivePhoto * _Nullable livePhoto, NSDictionary * _Nullable info) {
            if (completion) {
...
- (NSInteger)requestPlayerItemWithCompletion:(void (^)(AVPlayerItem *playerItem, NSDictionary<NSString *, id> *info))completion withProgressHandler:(PHAssetVideoProgressHandler)phProgressHandler {
    if ([[PHCachingImageManager class] instancesRespondToSelector:@selector(requestPlayerItemForVideo:options:resultHandler:)]) {
        ...
        return [[[TTPhotoManager manager] phCachingImageManager] requestPlayerItemForVideo:_phAsset options:videoRequestOptions resultHandler:^(AVPlayerItem * _Nullable playerItem, NSDictionary * _Nullable info) {
            if (completion) {
...
- (void)requestImageData:(void (^)(NSData *imageData, NSDictionary<NSString *, id> *info, BOOL isGIF, BOOL isHEIC))completion {
    if (self.assetType != TTAssetTypeImage) {
        if (completion) {
    ...
    if (!self.phAssetInfo) {
        ...
        [self requestPhAssetInfo:^(NSDictionary *phAssetInfo) {
            ...
            if (completion) {
                ...
                BOOL isGIF = self.assetSubType == TTAssetSubTypeGIF;
...
- (void)requestPhAssetInfo:(void (^)(NSDictionary *))completion {
    ...
    if (self.assetType == TTAssetTypeVideo) {
        PHVideoRequestOptions *videoRequestOptions = [[PHVideoRequestOptions alloc] init];
        videoRequestOptions.networkAccessAllowed = YES;
        [[[TTPhotoManager manager] phCachingImageManager] requestAVAssetForVideo:_phAsset options:videoRequestOptions resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
...
- (void)requestImagePhAssetInfo:(void (^)(NSDictionary *))completion synchronous:(BOOL)synchronous {
    ...
    imageRequestOptions.networkAccessAllowed = YES;
    [[[TTPhotoManager manager] phCachingImageManager] requestImageDataForAsset:_phAsset options:imageRequestOptions resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        if (info) {
            NSMutableDictionary *tempInfo = [[NSMutableDictionary alloc] init];
            if (imageData) {
...
- (void)setDownloadProgress:(double)downloadProgress {
    ...
    _downloadStatus = TTAssetDownloadStatusDownloading;
}

- (void)updateDownloadStatusWithDownloadResult:(BOOL)succeed {
    _downloadStatus = succeed ? TTAssetDownloadStatusSucceed : TTAssetDownloadStatusFailed;
}
...
- (BOOL)isEqual:(id)object {
    ...
    return [self.identifier isEqualToString:((TTAsset *)object).identifier];
}
